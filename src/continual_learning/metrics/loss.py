...
class DataLossPluginMetric(GenericPluginMetric[float]):
    def __init__(self, reset_at, emit_at, mode, split_by_task=False):
        self.split_by_task = split_by_task
        if self.split_by_task:
            self._loss = TaskAwareLoss()
            ...
            self._loss = LossMetric()
        super(DataLossPluginMetric, self).__init__(
            self._loss, reset_at, emit_at, mode
        )
    ...
    def update(self, strategy):
        if self.split_by_task:
            self._loss.update(
                loss=strategy.data_loss,
                patterns=len(strategy.mb_y),
                task_label=strategy.mb_task_id,
            )
        else:
            self._loss.update(strategy.data_loss, patterns=len(strategy.mb_y))


class MinibatchDataLoss(DataLossPluginMetric):
    def __init__(self):
        super(MinibatchDataLoss, self).__init__(
            reset_at="iteration", emit_at="iteration", mode="train"
        )
...
class EpochDataLoss(DataLossPluginMetric):
    def __init__(self):
        super(EpochDataLoss, self).__init__(
            reset_at="epoch", emit_at="epoch", mode="train"
        )

    def __str__(self):
        return "Data_Loss_Epoch"


class RunningEpochDataLoss(DataLossPluginMetric):
    def __init__(self):
        super(RunningEpochDataLoss, self).__init__(
            reset_at="epoch", emit_at="iteration", mode="train"
        )

    def __str__(self):
        return "Data_RunningLoss_Epoch"


class ExperienceDataLoss(DataLossPluginMetric):
    def __init__(self):
        super(ExperienceDataLoss, self).__init__(
            reset_at="experience", emit_at="experience", mode="eval"
        )

    def __str__(self):
        return "Data_Loss_Exp"


class StreamDataLoss(DataLossPluginMetric):
    def __init__(self):
        super(StreamDataLoss, self).__init__(
            reset_at="stream", emit_at="stream", mode="eval"
        )

    def __str__(self):
        return "Data_Loss_Stream"
    

class ReplayLossPluginMetric(GenericPluginMetric[float]):
    def __init__(self, reset_at, emit_at, mode, split_by_task=False):
        self.split_by_task = split_by_task
        if self.split_by_task:
            self._loss = TaskAwareLoss()
            ...
            self._loss = LossMetric()
        super(ReplayLossPluginMetric, self).__init__(
            self._loss, reset_at, emit_at, mode
        )
    ...
    def update(self, strategy):
        if self.split_by_task:
            self._loss.update(
                loss=strategy.replay_loss,
                patterns=len(strategy.mb_y),
                task_label=strategy.mb_task_id,
            )
        else:
            self._loss.update(strategy.replay_loss, patterns=len(strategy.mb_y))


class MinibatchReplayLoss(ReplayLossPluginMetric):
    def __init__(self):
        super(MinibatchReplayLoss, self).__init__(
            reset_at="iteration", emit_at="iteration", mode="train"
        )

    def __str__(self):
        return "Replay_Loss_MB"


class EpochReplayLoss(ReplayLossPluginMetric):
    def __init__(self):
        super(EpochReplayLoss, self).__init__(
            reset_at="epoch", emit_at="epoch", mode="train"
        )

    def __str__(self):
        return "Replay_Loss_Epoch"


class RunningEpochReplayLoss(ReplayLossPluginMetric):
    def __init__(self):
        super(RunningEpochReplayLoss, self).__init__(
            reset_at="epoch", emit_at="iteration", mode="train"
        )
...
class ExperienceReplayLoss(ReplayLossPluginMetric):
    def __init__(self):
        super(ExperienceReplayLoss, self).__init__(
            reset_at="experience", emit_at="experience", mode="eval"
        )

    def __str__(self):
        return "Replay_Loss_Exp"


class StreamReplayLoss(ReplayLossPluginMetric):
    def __init__(self):
        super(StreamReplayLoss, self).__init__(
            reset_at="stream", emit_at="stream", mode="eval"
        )

    def __str__(self):
        return "Replay_Loss_Stream"
...
def data_loss_metrics(
    *,
    ...
    metrics = []
    if minibatch:
        metrics.append(MinibatchDataLoss())

    if epoch:
        metrics.append(EpochDataLoss())

    if epoch_running:
        metrics.append(RunningEpochDataLoss())

    if experience:
        metrics.append(ExperienceDataLoss())

    if stream:
        metrics.append(StreamDataLoss())

    return metrics


def replay_loss_metrics(
    *,
    minibatch=False,
    epoch=False,
    epoch_running=False,
    ...
    metrics = []
    if minibatch:
        metrics.append(MinibatchReplayLoss())

    if epoch:
        metrics.append(EpochReplayLoss())

    if epoch_running:
        metrics.append(RunningEpochReplayLoss())

    if experience:
        metrics.append(ExperienceReplayLoss())

    if stream:
        metrics.append(StreamReplayLoss())
...
__all__ = [
    ...
    "ExperienceLoss",
    "StreamLoss",
    "DataLossPluginMetric",
    "MinibatchDataLoss",
    "EpochDataLoss",
    "RunningEpochDataLoss",
    "ExperienceDataLoss",
    "StreamDataLoss",
    "ReplayLossPluginMetric",
    "MinibatchReplayLoss",
    "EpochReplayLoss",
    "RunningEpochReplayLoss",
    "ExperienceReplayLoss",
    "StreamReplayLoss",
    "loss_metrics",
    "data_loss_metrics",
    "replay_loss_metrics",
